...
<body>
    ...
    <form action="/update/{{$comment->id}}" method="post" class="komen">
        @csrf
        @method('PATCH')
        ...
        <div class="rating1" id="userRating">
            ...
            <input value="{{ $comment->bintang}}" type="hidden" name="bintang" id="bintang">
        </div>
        <center>
            <div class="comment-form">
                <input value="{{ $comment->nama}}" type="text" class="usn" placeholder="Username" name="nama" required>
                <textarea id="comment" class="comment-text" name="comment" rows="10" placeholder="Type here..." required>{{ $comment->comment}}</textarea>
            </div>
        </center>
    </form>
